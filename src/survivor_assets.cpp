...
void AssetsLoad(Assets* assets, Asset id)
{
    ...
    // if (id >= Texture_Zombie && id <= Texture_Fence)
    // if (id >= Texture_Zombie && id <= Texture_Crosshair)
        ...
        if (file.content)
        {
            ...
            case AssetType_Model:
            {
                ...
                for (u32 meshIndex = 0; meshIndex < model->meshCount; meshIndex++)
                {
                    ...
                    // {
                    //     glm::vec3 position = mesh->vertexs[vertexIndex].position;
                    //
                    //     mesh->aabb.min.x = Min(mesh->aabb.min.x, position.x);
                    //     mesh->aabb.min.y = Min(mesh->aabb.min.y, position.y);
                    //     mesh->aabb.min.z = Min(mesh->aabb.min.z, position.z);
                    //
                    //     mesh->aabb.max.x = Max(mesh->aabb.max.x, position.x);
                    //     mesh->aabb.max.y = Max(mesh->aabb.max.y, position.y);
                    //     mesh->aabb.max.z = Max(mesh->aabb.max.z, position.z);
                    // }
            ...
            case AssetType_Animation:
            {
                ...
                // StreamRead(&stream, header.name, sizeof(header.name), 1);
